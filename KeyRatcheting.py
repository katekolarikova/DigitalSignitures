...
def derive_ratchein_key(kchain, ratchet_label="ratchet"):
...
def derive_message_key(kchain,  ratchet_label="message"):
    """Derives a message key from the current chain key."""
    message_key = hmac.new(kchain, ratchet_label.encode(), hashlib.sha256).digest()
